...
-export([start_link/2, send/2, token_received/2, stop/1, command/2]).
...
init([Address, Port]) ->
	...
	spawn_link(?MODULE, read_input_loop, [self()]),
    {ok, #state{connection = ConnPid}}.

handle_cast({send, Message}, #state{connection = Conn} = S) ->
    mudec_connection:send(Conn, [Message, "\r\n"]),
    {noreply, S};
handle_cast({command, "ping"}, #state{connection = Conn} = S) ->
    mudec_connection:send(Conn, [?IAC, ?AYT]),
    {noreply, S};
...
read_input_loop(Pid) ->
    ...
    case Input of
	[$/, Command] ->
	    mudec_console:command(Pid, Command);
	_ ->
	    mudec_console:send(Pid, Input)
    end,
